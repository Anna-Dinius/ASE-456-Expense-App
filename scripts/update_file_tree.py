...
EXCLUDE_DIRS = {'.vscode', '.git', '.idea', '.gradle', 'build', '.dart_tool', '__pycache__', 'android', 'coverage', 'ios', 'linux', 'macos', 'web', 'windows', 'assets', 'screenshots', 'pic'}
EXCLUDE_FILES = {'pubspec.lock', '.DS_Store', '.flutter-plugins-dependencies', '.gitignore', '.metadata', 'serviceAccountKey.json', 'firepit-log.txt'}
EXCLUDE_FILE_TYPES = {'.iml', '.log', '.mp4'}
...
def is_excluded(path: Path) -> bool:
    ...
    if name in EXCLUDE_FILES:
        return True
    if suffix in EXCLUDE_FILE_TYPES:
        return True
    return False
