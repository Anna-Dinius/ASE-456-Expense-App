...
class BudgetService {
  ...
  static Future<List<Budget>> getActiveBudgets(String userId) async {
    try {
      ...
      debugPrint('Error getting active budgets: $e');
      return [];
    }
  }
  ...
  static Future<List<Budget>> getBudgetsForCategory(
    ...
    try {
      ...
      debugPrint('Error getting budgets for category: $e');
      return [];
    }
  }
  ...
  static Future<Budget?> createBudget({
    ...
    try {
      ...
      if (overlappingBudgets.isNotEmpty) {
        ...
        throw Exception(
            'Budget overlaps with existing budget(s): $budgetNames. Please adjust the date range or deactivate the overlapping budget(s).');
      }
  ...
  static Future<bool> deleteBudget({
    ...
    try {
      ...
      debugPrint('Successfully deleted budget $budgetId');
      return true;
    } catch (e) {
  ...
  static Future<void> seedDefaultBudgets(String userId) async {
    try {
      ...
      if (existingBudgets.isNotEmpty) {
        debugPrint('Budgets already exist for user $userId');
        return;
      }
      ...
      await batch.commit();
      debugPrint('Successfully seeded default budgets for user $userId');
    } catch (e) {
      debugPrint('Error seeding default budgets: $e');
      rethrow;
    }
  }
  ...
  static Future<double> calculateRemainingAmount(
      String userId, Budget budget) async {
  ...
  static Future<double> calculateUtilizationPercentage(
      String userId, Budget budget) async {
    ...
    try {
      ...
      debugPrint('Error calculating utilization percentage: $e');
      return 0.0;
    }
  }
  ...
  static Future<List<Budget>> getBudgetsNeedingAttention(String userId) async {
    try {
      ...
      for (final budget in activeBudgets) {
        final utilization =
            await calculateUtilizationPercentage(userId, budget);
      ...
      debugPrint('Error getting budgets needing attention: $e');
      return [];
    }
  }
  ...
  static Future<List<Budget>> _findOverlappingBudgets(
    ...
    try {
      ...
      for (final budget in allBudgets) {
        ...
        if (startDate.isBefore(budget.endDate) &&
      ...
      debugPrint('Error finding overlapping budgets: $e');
      return [];
    }
  }
  ...
  static BudgetValidationResult _validateBudget(
    ...
    if (name.trim().length > 100) {
      return BudgetValidationResult(
          false, 'Budget name must be 100 characters or less');
    }

    // Check amount is positive
    if (amount <= 0) {
      return BudgetValidationResult(
          false, 'Budget amount must be greater than 0');
    }

    // Check amount is reasonable (not too large)
    if (amount > 1000000) {
      return BudgetValidationResult(
          false, 'Budget amount cannot exceed \$1,000,000');
    }
    ...
    if (endDate.difference(startDate).inDays > 1825) {
      return BudgetValidationResult(
          false, 'Budget period cannot exceed 5 years');
    }
    ...
    if (type == BudgetType.CATEGORY &&
      ...
      return BudgetValidationResult(
          false, 'Category must be specified for category-specific budgets');
    }
    ...
    for (final threshold in alertThresholds) {
      if (threshold < 0 || threshold > 1) {
        return BudgetValidationResult(
            false, 'Alert thresholds must be between 0 and 1');
      }
    }
    ...
    for (int i = 1; i < alertThresholds.length; i++) {
      if (alertThresholds[i] <= alertThresholds[i - 1]) {
        return BudgetValidationResult(
            false, 'Alert thresholds must be in ascending order');
      }
    }
  ...
  static Future<int> getTransactionCountForBudget(
    ...
    try {
      ...
      debugPrint('Error getting transaction count for budget: $e');
      return 0;
    }
  }
}
